    ...
    try {
        ...
        for (const row of data) {
            ...
            if (!row.name || !row.event_name || !row.issue_date) {
                skipped.push({ row, reason: "Missing required fields (name, event_name, or issue_date)" });
                continue;
            }
            ...
            if (email) {
                ...
                if (existing) {
                    skipped.push({ row, reason: `Duplicate: ${email} already has a certificate for ${event_name}` });
                    continue;
                }
            }
