...
export async function GET() {
  try {
    ...
    const { data: eventData } = await supabase
      ...
      .select("event_name");
    const uniqueEvents = [
      ...new Set((eventData || []).map((r) => r.event_name)),
    ].sort();
    ...
    switch (action) {
      ...
      case "revoke_all": {
        ...
        return NextResponse.json({
          message: `Revoked all active certificates`,
        });
      }
      case "restore_all": {
        const { error } = await supabase
        ...
        if (error) throw error;
        return NextResponse.json({
          message: `Restored all revoked certificates`,
        });
      }
      case "delete_event": {
        if (!eventName) {
        ...
        if (error) throw error;
        return NextResponse.json({
          message: `Deleted certificates for event "${eventName}"`,
        });
      }
      default:
        return NextResponse.json(
