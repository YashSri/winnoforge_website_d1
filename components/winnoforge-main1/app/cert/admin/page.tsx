...
export default function AdminDashboard() {
  ...
  const toggleRevoke = async (id: string) => {
    try {
      const res = await fetch(`/api/cert/certificates/${id}/revoke`, {
        method: "PATCH",
      });
      if (res.ok) {
        fetchStats();
  ...
  const skippedDetails = uploadResult?.skippedDetails ?? [];
  ...
  return (
    <div className="admin-container fade-in">
        ...
        <div className="header-content">
          ...
          <div>
            ...
            <p className="header-subtitle">
              Upload CSV files to issue certificates in bulk via Supabase.
            </p>
          </div>
        </div>
      </div>
      ...
      <div
        className="upload-section fade-in-up"
        style={{ animationDelay: "0.4s" }}
      >
        <h2 className="section-title">Issue New Certificates</h2>
        <label
          htmlFor="cert-csv-input"
            ...
            {file ? (
              ...
              <>
                ...
                <p className="dropzone-hint">
                  CSV files only (name, email, event_name, issue_date)
                </p>
              </>
            )}
          </div>
                ...
                {skippedDetails.length > 0 && (
                  <div className="skipped-details">
                    ...
                    {skippedDetails.map((s) => (
                      <div
                        key={`${s.row.name || "unknown"}-${s.row.event_name || "event"}-${s.reason}`}
                        className="skipped-row"
                      >
                        <span className="skipped-name">
                          {s.row.name || "Unknown"} (
                          {s.row.event_name || "Unknown Event"})
                        </span>
      ...
      {certificates.length > 0 ? (
        <div
          className="table-section fade-in-up"
            ...
            <table className="data-table">
              ...
              <tbody>
                {certificates.map((cert) => (
                  <tr
                    key={cert.id}
                    className={cert.is_revoked === 1 ? "row-revoked" : ""}
                  >
                    <td>
        ...
        <div
          className="table-section empty-state fade-in-up"
          style={{ animationDelay: "0.5s" }}
        >
          <div className="upload-icon" style={{ opacity: 0.5 }}>
            <Icons.File />
          </div>
      ...
      {/* Super Admin Options */}
      <div
        className="superadmin-section fade-in-up"
          ...
          <div className="superadmin-panel fade-in">
            <div className="sa-btn-grid" style={{ marginTop: "16px" }}>
              <button
                ...
                onClick={() => executeAdminAction("revoke_all")}
              >
                Revoke All
              </button>
              <button
                ...
                onClick={() => executeAdminAction("restore_all")}
              >
                Restore All
              </button>
              <button
                ...
                onClick={() => executeAdminAction("clear_all")}
              >
                Clear Records
              </button>
              <button
                type="button"
                className="sa-btn sa-btn-danger"
                onClick={() => executeAdminAction("drop_db")}
              >
                Drop DB Items
              </button>
            </div>
            ...
            {events.length > 0 && (
              <div style={{ marginTop: "24px" }}>
                ...
                <div className="sa-event-list">
                  {events.map((evt) => (
                    <div className="sa-event-row" key={evt}>
                      <span className="sa-event-name">{evt}</span>
                      <button
                        type="button"
                        className="sa-btn sa-btn-sm sa-btn-danger"
                        onClick={() => executeAdminAction("delete_event", evt)}
                      >
                        Delete Event
                      </button>
                    </div>
      ...
      {qrModal.isOpen && (
        <div className="modal-overlay">
          <button
            ...
            onClick={() => setQrModal({ isOpen: false, qrCode: "", url: "" })}
          />
          <div className="modal relative z-10" role="dialog" aria-modal="true">
            <button
              type="button"
            ...
            <a
              ...
              className="qr-url block text-center truncate hover:underline text-indigo-500 mt-4"
            >
              {qrModal.url}
            </a>
          </div>
